...
pub mod modern_ciphers{
    pub mod stream_cipher{
        ...
        pub fn step1(sk:[u8;64]) -> (Vec<u8>,Vec<u8>,Vec<u8>){
            //Sessionkey is mixed into the LFSR's with no majority rule
            let (mut lfsr1,mut lfsr2,mut lfsr3) = init();
            let mut count = 0;
            for i in sk.iter(){
        ...
        pub fn step2(frame:[u8;22],mut lfsr1:Vec<u8>, mut lfsr2:Vec<u8>, mut lfsr3:Vec<u8>) -> (Vec<u8>,Vec<u8>,Vec<u8>){
        ...
        pub fn step3(mut lfsr1: Vec<u8>,mut lfsr2: Vec<u8>,mut lfsr3: Vec<u8>) -> (Vec<u8>,Vec<u8>,Vec<u8>){
        ...
        pub fn step4(mut lfsr1: Vec<u8>,mut lfsr2: Vec<u8>,mut lfsr3: Vec<u8>) -> Vec<u8>{
        ...
        pub fn write_file(vec: Vec<u8>) ->  std::io::Result<()>{
            use std::fs;
            use std::io::Write;
            let mut file = fs::File::create("WUFFER.txt")?;
            for i in vec{
        ...
        pub fn write_file_2(vec: Vec<u8>) ->  std::io::Result<()>{
            use std::fs;
            use std::io::Write;
            let mut file = fs::File::create("WUFFER2.txt")?;
            for i in vec{
                let _write = file.write(&[i]);
            }
            Ok(())
